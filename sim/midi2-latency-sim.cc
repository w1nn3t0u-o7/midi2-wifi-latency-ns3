...
int main(int argc, char *argv[]) {
  ...
  cmd.AddValue("standard", "Wi-Fi standard: 80211ac, 80211ax, 80211be",
               standard);
  ...
  cmd.AddValue("muSchedAccessReqInterval",
               ...
               accessReqInterval);
  cmd.AddValue("mcs",
               "list of comma separated MCS values to test; if unset, all MCS "
               "values (0-11) are tested",
               mcsStr);
  cmd.AddValue("channelWidth",
               "if set, limit testing to a specific channel width expressed in "
               "MHz (20, 40, 80 "
               "or 160 MHz)",
               channelWidth);
  cmd.AddValue("guardInterval",
               "if set, limit testing to a specific guard interval duration "
               "expressed in "
  ...
  if (mcsStr.empty()) {
    ...
    auto checker = DynamicCast<AttributeContainerChecker>(
        MakeAttributeContainerChecker(attr));
  ...
  for (const auto mcs : mcsValues) {
    for (int width = minChannelWidth; width <= maxChannelWidth;
         width *= 2) // MHz
    {
      ...
      for (int gi = maxGi; gi >= minGi; gi /= 2) // Nanoseconds
      {
        ...
        if (standard == "80211ac")
          wifiStd = WIFI_STANDARD_80211ac;
        else if (standard == "80211ax")
          wifiStd = WIFI_STANDARD_80211ax;
        else if (standard == "80211be")
          wifiStd = WIFI_STANDARD_80211be;
        else
          NS_FATAL_ERROR("Invalid standard: " << standard);
        ...
        if (standard == "80211ac") {
          ...
          wifi.SetRemoteStationManager(
              ...
              StringValue(ossDataMode.str()), "ControlMode", ctrlRate);
          wifi.ConfigHtOptions("ShortGuardIntervalSupported",
                               BooleanValue(gi == 400));
        ...
        if (phyModel == "Spectrum") {
          ...
          mac.SetType("ns3::ApWifiMac", "EnableBeaconJitter",
                      BooleanValue(false), "BeaconGeneration",
                      BooleanValue(!staticSetup), "Ssid", SsidValue(ssid));
        ...
        Ptr<ListPositionAllocator> apPos =
        ...
        for (std::size_t i = 0; i < nMidi; i++) {
          ...
          if (midiModel == "onoff") {
            ...
            streamNumber +=
                client.AssignStreams(midiNodes.Get(i), streamNumber);
          }
        ...
        std::cout << "FlowID\tSrc\t\tDst\t\ttxPkts\trxPkts\tlost\tlossrate\tmea"
                     "nDelay(ms)\tjitter(ms)\n";

        for (auto &[id, fs] : stats) {
          auto ft = classifier->FindFlow(id);

          if (ft.destinationPort != midiPort)
            continue;

          double meanDelay = fs.rxPackets > 0 ? fs.delaySum.GetSeconds() /
                                                    fs.rxPackets * 1000.0
                                              : -1.0;
          double meanJitter = fs.rxPackets > 1 ? fs.jitterSum.GetSeconds() /
                                                     (fs.rxPackets - 1) * 1000.0
                                               : -1.0;
          double lossRate =
              100.0 * fs.lostPackets / std::max(uint32_t(1), fs.txPackets);

          std::cout << id << "\t" << ft.sourceAddress << ":" << ft.sourcePort
                    << "\t" << ft.destinationAddress << ":"
                    << ft.destinationPort << "\t" << fs.txPackets << "\t"
                    << fs.rxPackets << "\t" << fs.lostPackets << "\t"
                    << lossRate << "%\t" << meanDelay << "\t\t" << meanJitter
                    << "\n";
        }
