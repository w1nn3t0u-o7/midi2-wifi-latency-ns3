...
STD_LABELS = {"80211ac": "Wi-Fi 5 (802.11ac)",
...
def add_thresholds(ax):
    ax.axhline(LATENCY_LIMIT,   color="tomato", linestyle="--", linewidth=1.2,
...
def aggregate_with_ci(df, param_col="param_value"):
    ...
    per_run = (df.groupby(["standard", param_col, "rng_run"])
                 .agg(delay =("meanDelay_ms",  "mean"),
    ...
    for (std, pval), grp in per_run.groupby(["standard", param_col]):
        ...
        for metric, col in [("delay", "delay"), ("jitter", "jitter"), ("loss", "loss")]:
            ...
            if len(vals) >= 2:
                ...
                h = np.nan
            records.append({"standard": std, param_col: pval,
                             f"mean_{metric}": mean, f"ci_{metric}": h})
...
# ---------------------------------------------------------------
def plot_scenario(csv_name, scenario_title, xlabel, x_col,
                  output_name, marker="o", x_integer=True):
    ...
    for ax, (col, ci_col, ylabel) in zip(axes, metrics):
        for std in STANDARDS:
            ...
            ax.plot(x, y, marker=marker, label=STD_LABELS[std],
                    color=STD_COLORS[std], linewidth=2, markersize=6, zorder=3)
            ...
            if valid.any():
                ax.fill_between(x[valid],
                                (y - ci)[valid],
                                (y + ci)[valid],
                                color=STD_COLORS[std], alpha=0.15, zorder=2)
...
def plot_overview():
    configs = [
        ("scenario1_capacity.csv",  "param_value", "Number of MIDI Nodes",   "Capacity (nMidi)"),
        ("scenario2_background.csv","param_value", "Background Nodes",        "Background (nBG)"),
        ("scenario3_distance.csv",  "param_value", "Distance (m)",            "Distance (m)"),
    ]

    fig, axes = plt.subplots(1, 3, figsize=(17, 5))
    fig.suptitle("MIDI 2.0 over 802.11 — Mean Delay Overview (95% CI)", fontsize=14,
                 fontweight="bold")
    ...
    for ax, (fname, xcol, xlabel, title) in zip(axes, configs):
        ...
        for std in STANDARDS:
            ...
            ax.plot(x, y, marker="o", label=STD_LABELS[std],
                    color=STD_COLORS[std], linewidth=2.2, markersize=6, zorder=3)
            valid = ~np.isnan(ci) & ~np.isnan(y)
            if valid.any():
                ax.fill_between(x[valid], (y - ci)[valid], (y + ci)[valid],
                                color=STD_COLORS[std], alpha=0.15, zorder=2)
        add_thresholds(ax)
